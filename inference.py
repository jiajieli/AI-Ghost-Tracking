    ...
    def elapseTime(self, gameState):
        ...
        *** GORY DETAIL AHEAD ***
        ...
        for oldPos in self.legalPositions:
            newPosDist = self.getPositionDistribution(self.setGhostPosition(gameState, oldPos))
            for newPos, prob in newPosDist.items():
                allPossibles[newPos] += prob * self.beliefs[oldPos]
...
class ParticleFilter(InferenceModule):
    ...
    def observe(self, observation, gameState):
        ...
        if noisyDistance is None:
            ...
            if allPossible.totalCount() == 0:
                ...
                allPossible.normalize()
                self.particleList = []
                distribution = [i[1] for i in allPossible.items()]
                values = [i[0] for i in allPossible.items()]
                self.particleList = util.nSample(distribution, values, self.numParticles)
    ...
    def getBeliefDistribution(self):
        ...
        belief = util.Counter()
        for i in self.particleList:
            belief[i] += 1.0/self.numParticles
        return belief
...
class JointParticleFilter:
    ...
    def initializeParticles(self):
        ...
        self.particles = []
        numP = len(permutations)
        numN = self.numParticles
        for _ in range(numN / numP):
            self.particles += permutations
        self.particles += permutations[0: numN - numN / numP * numP - 1]
    ...
    def getBeliefDistribution(self):
        belief = util.Counter()
        for p in self.particles:
            belief[p] += 1.0/self.numParticles
        return belief
